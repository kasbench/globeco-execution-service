...
class ExecutionServiceImplTest {
    ...
    void testSaveAndFind() {
        Execution execution = new Execution(null, "NEW", "BUY", "NYSE", new BigDecimal("200.00"), new BigDecimal("20.00"), OffsetDateTime.now(), null, null);
    ...
    void testDeleteWithOptimisticLocking() {
        Execution execution = new Execution(null, "NEW", "SELL", "NASDAQ", new BigDecimal("75.00"), null, OffsetDateTime.now(), null, null);
        Execution saved = executionService.save(execution);
        // Should succeed
        executionService.deleteById(saved.getId(), saved.getVersion());
        // Should throw if version is wrong
        Execution another = new Execution(null, "NEW", "SELL", "NASDAQ", new BigDecimal("75.00"), null, OffsetDateTime.now(), null, null);
